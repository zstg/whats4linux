...
export function MediaContent({ message, type, chatId, sentMediaCache, onImageClick, onDownload }: MediaContentProps) {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      setMediaSrc(`data:${(message.Content as any)?.[`${type}Message`]?.mimetype || "application/octet-stream"};base64,${data}`)
    } catch (e) { }
    finally { setLoading(false) }
  }
  ...
  if (mediaSrc) {
    if (type === "image" || type === "sticker") {
      return (
        <div className="relative inline-block" onMouseEnter={() => type === "image" && setShowDownloadButton(true)} onMouseLeave={() => setShowDownloadButton(false)}>
          <img 
            src={mediaSrc} 
            className={type === "image" ? "max-w-full rounded-lg cursor-pointer" : "object-contain w-48.75 h-48.75"} 
            alt="media" 
            onClick={type === "image" && onImageClick ? () => onImageClick(mediaSrc) : undefined}
          />
          {type === "image" && showDownloadButton && onDownload && (
            <button onClick={(e) => { e.stopPropagation(); onDownload() }} className="absolute top-2 right-2 p-2 bg-black/70 hover:bg-black/90 rounded-full text-white transition-colors" title="Download image">
  ...
  return (
    <div className="w-64 h-64 bg-gray-200 dark:bg-gray-800 rounded-lg flex items-center justify-center">
      {loading ? <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-green-500" /> : (
        <button onClick={type === "video" || type === "audio" ? handleDownload : loadFromCache} className="bg-black/50 p-3 rounded-full text-white hover:bg-black/70">
